...
export const OrderScreen = () => {
    ...
    async function deliverOrderHandler() {
        try {
            ...
            // מייל ללקוח על אישור שליחת הזמנה
            window.Email.send({
                Host: "smtp.elasticemail.com",
                Username: `${process.env.REACT_APP_MAIL_USERNAME}`,
                Password: `${process.env.REACT_APP_MAIL_PASSWORD}`,
    ...
    useEffect(() => {
        const fetchOrder = async () => {
            // נגדיר את useEffect כך שברגע שנגיע למסך זה, תשלח בקשה לצד שרת עם axios לקבלת ההזמנה על פי מזהה ההזמנה.
            try {
        ...
        //ניצור תנאי אשר אומר אם אין פרטים של משתמש מהlocalStorage, ננווט את המשתמש לעמוד ההתחברות.
        if (!userInfo) {
            return navigate('/signin');
        }
        //אחרת, אם ההזמנה גם שווה לnull או אם מספר ההזמנה לא זהה לorderId נבצע את fetchOrder:
